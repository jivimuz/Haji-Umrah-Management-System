...
class PaymentController extends Controller
{
    ...
    public function getAgenFee(Request $request)
    {
        $htp =    Agen::select([
            ...
            ->join(DB::raw("(SELECT COALESCE(SUM(publish_price - basic_price), 0) as fee,agen_id,paket_id, m_paket.nama as paket  FROM t_jamaah join m_paket on m_paket.id = t_jamaah.paket_id where agen_id = '$request->id' and publish_price - basic_price > 0 group by agen_id,paket_id) as dt"), 'dt.agen_id', 'm_agen.id')
